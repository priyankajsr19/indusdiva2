	...
	/*if( $cv > 50) {	
       ...
       //buy1 get 1
        ...
        foreach($products as $product) {   
            ...
            } else if(in_array('buy2get3', $tags)){
       ...
       ///echo "<pre>";print_r($offerproducts);
        ...
        foreach ($allCartDiscounts as $cartDiscount) {
            if(strpos($cartDiscount['name'], 'B1G1') === 0){
                ...
                //echo "buy1get1";
            } else if(strpos($cartDiscount['name'], 'B2G3') === 0){
    ...
    if(in_array('buy1get1', $tags))
      {
        if($discount > 0 && empty($b1g1DiscountInCart)){
            ...
            $new_discount->date_to = '2015-08-10 20:00:00'; //validity
...
else if(in_array('buy2get3', $tags))
{
        if($discount > 0 && empty($b2g3DiscountInCart)){
            ...
            $languages = Language::getLanguages();
            foreach ($languages as $language)
                $new_discount->description[$language['id_lang']] = "Buy2 Get3"; // coupon description
            
            $new_discount->add();
            $id_discount = Discount::getIdByName($new_discount->name);
            self::$cart->addDiscount($id_discount);
            
        }else if(!empty($b2g3DiscountInCart)){
            $b2g3DiscountInCart->value = (float) $discount; // coupon value
