...
$sql = "select cp.id_product from ps_category_product cp  
                        ...
                        where id_category = 4 and p.active = 1 and p.quantity > 0 and p.is_exclusive = 1 and p.is_customizable = 0";
$res = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS($sql);

$skus = getSKUs($res);
$header = array('id','name','parent_id', 'variation','size','description','quantity','price','indusdiva_code','salwar style','kameez style','sleeve style', 'keywords','shipping sla','color','generic color','fabric','work type(pattern)','wash care','category', "occasion","region", 'pattern', 'main image','other image 1','other image 2', 'other image 3','other image 4');
$cdata = tocsv($header) . "\n";
foreach($skus as $sku) {
	...
	$data[] = empty($product->salwar_style)?'':$product->salwar_style;
	$data[] = empty($product->kameez_style)?'':$product->kameez_style;
	$data[] = empty($product->sleeves)?'':$product->sleeves;
	...
	$this_occasion = array();
	if( in_array(99, $id_categories) )
		array_push($this_occasion, 'Casual');
	if( in_array(100, $id_categories) )
		array_push($this_occasion, 'Formal');
	if( in_array(144, $id_categories) ) {
		array_push($this_occasion, 'Semi Formal');
		array_push($this_occasion, 'Casual');
        }
	if( in_array(101, $id_categories) )
