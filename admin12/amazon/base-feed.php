...
class BaseFeed {
    ...
    protected function getProductsSQL() {
       ...
       // $cat_sql = array();
        ...
        $cat_sql[] = $sql;
        // skd
        $sql = "select cp.id_product from ps_category_product cp inner join ps_product p on p.id_product = cp.id_product  where cp.id_category = ".CAT_SKD;
        if ( isset($this->date_res) && !empty($this->date_res['last_date_add'])  ) 
            $sql .= " and (p.date_upd >= '".$this->date_res['last_date_add']."' OR p.date_add >= '".$this->date_res['last_date_add']."')";
        $cat_sql[] = $sql;

        //kurtis
        $sql = "select cp.id_product from ps_category_product cp inner join ps_product p on p.id_product = cp.id_product  where cp.id_category = ".CAT_KURTI;
        if ( isset($this->date_res) && !empty($this->date_res['last_date_add'])  ) 
            $sql .= " and (p.date_upd >= '".$this->date_res['last_date_add']."' OR p.date_add >= '".$this->date_res['last_date_add']."')";
        $cat_sql[] = $sql;
      ...
      $cat_sql[] = $sql;
        ...
        //lehengas
        $sql = "select cp.id_product from ps_category_product cp inner join ps_product p on p.id_product = cp.id_product  where cp.id_category = ".CAT_LEHENGA;
        if ( isset($this->date_res) && !empty($this->date_res['last_date_add'])  ) 
            $sql .= " and (p.date_upd >= '".$this->date_res['last_date_add']."' OR p.date_add >= '".$this->date_res['last_date_add']."')";
       $cat_sql[] = $sql;

	//menswear
	$sql = "select cp.id_product from ps_category_product cp inner join ps_product p on p.id_product = cp.id_product  where cp.id_category = ".CAT_MEN;
        if ( isset($this->date_res) && !empty($this->date_res['last_date_add'])  ) 
            $sql .= " and (p.date_upd >= '".$this->date_res['last_date_add']."' OR p.date_add >= '".$this->date_res['last_date_add']."')";
        $cat_sql[] = $sql;

	//kidswear
        $sql = "select cp.id_product from ps_category_product cp inner join ps_product p on p.id_product = cp.id_product  where cp.id_category = ".CAT_KIDS;
        if ( isset($this->date_res) && !empty($this->date_res['last_date_add'])  ) 
            $sql .= " and (p.date_upd >= '".$this->date_res['last_date_add']."' OR p.date_add >= '".$this->date_res['last_date_add']."')";
        $cat_sql[] = $sql;
...
//	$sql = "select id_product from temp1";
   //  $cat_sql[] = $sql;
    ...
    protected function getProductsSQL2() {
        $sql = "select max(date_add) last_date_add from ps_affiliate_feed";
        $this->date_res = $this->db->getRow($sql); 
        ...
        $cat_sql = array();
        // sarees
        $dates_sql = " and (p.date_upd >= '2015-05-15 00:00:00' OR p.date_add >= '2015-05-15 00:00:00') AND (p.date_upd <= '2015-05-16 23:59:59' OR p.date_add <= '2015-05-16 23:59:59')";
 ...
 //       $cat_sql[] = $sql;
	...
	if( $n ) {
            ...
            @Mail::Send(1, 'alert', Mail::l('Amazon Stock Sync Update'), $templateVars, array("venkatesh.padaki@violetbag.com","lekshmi.gopinathan@violetbag.com","vineet.saxena@violetbag.com","venugopal.annamaneni@violetbag.com","bharath.d@mantralabsglobal.com"), null, 'care@indusdiva.com', 'Indusdiva Monitoring', NULL, NULL, _PS_MAIL_DIR_, false);
        }
	...
	$this->id_affiliate = 1;
        ...
        if( $price > $specific_price && $specific_price > 0) {
